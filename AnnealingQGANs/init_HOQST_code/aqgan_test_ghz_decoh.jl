...
thetas = LinRange(0, 2*pi, 50)
θ = pi / 4
u0 = (cos(θ) * PauliVec[3][1] ⊗ PauliVec[3][1] ⊗ PauliVec[3][1] + sin(θ) *  PauliVec[3][2] ⊗ PauliVec[3][2] ⊗ PauliVec[3][2])
ρ0 = u0*u0'
tf = 5
H = DenseHamiltonian([(s) -> 1.0], [σi ⊗ σi ⊗ σi], unit=:ħ)
##

## Solve Schrodinger equation
annealing = Annealing(H, u0)
closed_sol = solve_schrodinger(annealing, tf, alg=Tsit5(), abstol=1e-6, reltol=1e-6);
closed_fid = abs(dot(u0, closed_sol[end]))^2
##

## Solve Lindblad for different rates
rates = [1e-2, 1e-1/4, 1]
lind_sols = []
lind_fids = []
for γ in rates
    ...
    push!(lind_sols, lind_sol)
    lind_fid = real(u0' * lind_sol[end] * u0)
    push!(lind_fids, lind_fid)
end
##

##
ls = lind_sols[1]
x1 = LinRange(0, tf, length(ls))
fid1 = [fidelity(ls[j], ρ0) for j=1:length(ls)]
ls = lind_sols[2]
x2 = LinRange(0, tf, length(ls))
fid2 = [fidelity(ls[j], ρ0) for j=1:length(ls)]
ls = lind_sols[3]
x3 = LinRange(0, tf, length(ls))
fid3 = [fidelity(ls[j], ρ0) for j=1:length(ls)]
##

##
plot(x1, fid1, label="γ=0.01", xlabel="t", ylabel="F(ρ₀,ρ(t))")
plot!(x2, fid2, label="γ=0.025")
plot!(x3, fid3, label="γ=0.1")
##
